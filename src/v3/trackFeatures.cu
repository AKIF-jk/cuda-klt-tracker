...
// Add any other C headers here if they're included in trackFeatures.cu
...
#include "klt.h"
#include "klt_util.h"
#include "error.h"   /* For KLTError if needed -- ensure available at link time */
...
/* Keep original typedef for windows */
...
__global__ void _compute2by2GradientMatrix_kernel_atomic(
    const float *gradx, const float *grady, int N,
    float *gxx, float *gxy, float *gyy)
{
    ...
    float local_gxx = 0.0f;
    float local_gxy = 0.0f;
    float local_gyy = 0.0f;

    /* strided loop so kernel covers all N elements even if grid < ceil(N/block) */
    ...
    /* Atomically add each local accumulator to the global totals */
    if (local_gxx != 0.0f) atomicAdd(gxx, local_gxx);
    if (local_gxy != 0.0f) atomicAdd(gxy, local_gxy);
    if (local_gyy != 0.0f) atomicAdd(gyy, local_gyy);
}
...
/***************************************************************************/
/* Helper: create a texture object from a host float image (row-major)      */
/* Returns texture object, and writes the created cudaArray to *outArray    */
/* so caller can destroy it with cudaFreeArray.                             */
/***************************************************************************/
static cudaTextureObject_t _createTextureFromFloatImage(_KLT_FloatImage img, cudaArray_t *outArray)
{
    /* Describe channel */
    ...
    KLT_CUDA_ASSERT(cudaMallocArray(&cuArray, &desc, img->ncols, img->nrows));

    /* Copy host memory (row-major float) to the cudaArray */
    size_t srcPitch = img->ncols * sizeof(float);
    KLT_CUDA_ASSERT(cudaMemcpy2DToArray(cuArray, 0, 0,
                img->data, srcPitch,
                srcPitch, img->nrows,
    ...
    KLT_CUDA_ASSERT(cudaCreateTextureObject(&tex, &resDesc, &texDesc, nullptr));

    if (outArray) *outArray = cuArray;
    return tex;
}

/***************************************************************************/
/* Optimized shared-memory GPU kernels for intensity difference and gradient sum */
/***************************************************************************/

#define TILE_W 32
#define TILE_H 16

__global__ void _k_computeIntensityDifference_shared(
    ...
    float *d_out)
{
    ...
    float *tile1 = sdata;
    float *tile2 = sdata + (TILE_W + 2) * (TILE_H + 2);
    ...
    sx1 = fminf(fmaxf(sx1, 0.0f), (float)(ncols - 1));
    sy1 = fminf(fmaxf(sy1, 0.0f), (float)(nrows - 1));
    sx2 = fminf(fmaxf(sx2, 0.0f), (float)(ncols - 1));
    sy2 = fminf(fmaxf(sy2, 0.0f), (float)(nrows - 1));

    // Each thread loads one pixel into shared memory
    int idx = (int)sy1 * ncols + (int)sx1;
    int idy = (int)sy2 * ncols + (int)sx2;

    if (gx < ncols && gy < nrows) {
        tile1[ty * (TILE_W + 2) + tx] = img1[idx];
        tile2[ty * (TILE_W + 2) + tx] = img2[idy];
    }
    __syncthreads();

    // Compute difference within the tile
    if (gx < width && gy < height)
        d_out[gy * width + gx] = tile1[ty * (TILE_W + 2) + tx] -
                                 tile2[ty * (TILE_W + 2) + tx];
}

__global__ void _k_computeGradientSum_shared(
...
{
    extern __shared__ float sdata[];
    float *tile_gx1 = sdata;
    float *tile_gy1 = sdata + (TILE_W + 2) * (TILE_H + 2);
    float *tile_gx2 = tile_gy1 + (TILE_W + 2) * (TILE_H + 2);
    float *tile_gy2 = tile_gx2 + (TILE_W + 2) * (TILE_H + 2);
    ...
    int idx1 = (int)sy1 * ncols + (int)sx1;
    int idx2 = (int)sy2 * ncols + (int)sx2;

    if (gx < ncols && gy < nrows) {
        tile_gx1[ty * (TILE_W + 2) + tx] = gradx1[idx1];
        tile_gy1[ty * (TILE_W + 2) + tx] = grady1[idx1];
        tile_gx2[ty * (TILE_W + 2) + tx] = gradx2[idx2];
        tile_gy2[ty * (TILE_W + 2) + tx] = grady2[idx2];
    }
    __syncthreads();

    if (gx < width && gy < height) {
        int outIdx = gy * width + gx;
        float gx_sum = tile_gx1[ty * (TILE_W + 2) + tx] + tile_gx2[ty * (TILE_W + 2) + tx];
        float gy_sum = tile_gy1[ty * (TILE_W + 2) + tx] + tile_gy2[ty * (TILE_W + 2) + tx];
        d_gx[outIdx] = gx_sum;
        d_gy[outIdx] = gy_sum;
    }
}
...
/***************************************************************************/
/***************************************************************************/
/* Host wrappers (Unified Memory Version)                                  */
/***************************************************************************/

extern "C" void _computeIntensityDifference(
  ...
  _FloatWindow imgdiff)
{
    ...
    size_t bytes = (size_t)ncols * nrows * sizeof(float);
    size_t winBytes = (size_t)width * height * sizeof(float);

    float *d_img1, *d_img2, *d_out;
    
    // 1. Allocate Unified Memory
    KLT_CUDA_ASSERT(cudaMallocManaged(&d_img1, bytes));
    KLT_CUDA_ASSERT(cudaMallocManaged(&d_img2, bytes));
    KLT_CUDA_ASSERT(cudaMallocManaged(&d_out, winBytes));

    // 2. Copy data to UM on the host
    memcpy(d_img1, img1->data, bytes);
    memcpy(d_img2, img2->data, bytes);

    // 3. Get device and prefetch data to GPU
    int deviceId;
    KLT_CUDA_ASSERT(cudaGetDevice(&deviceId));
    cudaStream_t stream = 0; // Use default stream

    KLT_CUDA_ASSERT(cudaMemPrefetchAsync(d_img1, bytes, deviceId, stream));
    KLT_CUDA_ASSERT(cudaMemPrefetchAsync(d_img2, bytes, deviceId, stream));
    KLT_CUDA_ASSERT(cudaMemPrefetchAsync(d_out, winBytes, deviceId, stream)); // Prefetch output buffer

    // 4. Launch kernel
    dim3 block(TILE_W, TILE_H);
    dim3 grid((width + TILE_W - 1) / TILE_W, (height + TILE_H - 1) / TILE_H);
    size_t sharedBytes = 2 * (TILE_W + 2) * (TILE_H + 2) * sizeof(float);

    _k_computeIntensityDifference_shared<<<grid, block, sharedBytes>>>(
        d_img1, d_img2, ncols, nrows, x1, y1, x2, y2, width, height, d_out);
    
    // 5. Wait for kernel to complete
    KLT_CUDA_ASSERT(cudaDeviceSynchronize());

    // 6. Prefetch results back to CPU
    KLT_CUDA_ASSERT(cudaMemPrefetchAsync(d_out, winBytes, cudaCpuDeviceId, stream));
    
    // 7. Wait for prefetch to CPU to complete
    KLT_CUDA_ASSERT(cudaDeviceSynchronize());

    // 8. Copy from UM (now on host) to output buffer
    memcpy(imgdiff, d_out, winBytes);
...
extern "C" void _computeGradientSum(
  ...
  _FloatWindow grady)
{
    ...
    float *d_gx1, *d_gy1, *d_gx2, *d_gy2, *d_outx, *d_outy;

    // 1. Allocate Unified Memory
    KLT_CUDA_ASSERT(cudaMallocManaged(&d_gx1, bytes));
    KLT_CUDA_ASSERT(cudaMallocManaged(&d_gy1, bytes));
    KLT_CUDA_ASSERT(cudaMallocManaged(&d_gx2, bytes));
    KLT_CUDA_ASSERT(cudaMallocManaged(&d_gy2, bytes));
    KLT_CUDA_ASSERT(cudaMallocManaged(&d_outx, winBytes));
    KLT_CUDA_ASSERT(cudaMallocManaged(&d_outy, winBytes));

    // 2. Copy data to UM on the host
    memcpy(d_gx1, gradx1->data, bytes);
    memcpy(d_gy1, grady1->data, bytes);
    memcpy(d_gx2, gradx2->data, bytes);
    memcpy(d_gy2, grady2->data, bytes);
    ...
    KLT_CUDA_ASSERT(cudaMemPrefetchAsync(d_gx1, bytes, deviceId, stream));
    KLT_CUDA_ASSERT(cudaMemPrefetchAsync(d_gy1, bytes, deviceId, stream));
    KLT_CUDA_ASSERT(cudaMemPrefetchAsync(d_gx2, bytes, deviceId, stream));
    KLT_CUDA_ASSERT(cudaMemPrefetchAsync(d_gy2, bytes, deviceId, stream));
    KLT_CUDA_ASSERT(cudaMemPrefetchAsync(d_outx, winBytes, deviceId, stream)); // Prefetch output
    KLT_CUDA_ASSERT(cudaMemPrefetchAsync(d_outy, winBytes, deviceId, stream)); // Prefetch output

    // 4. Launch kernel
    dim3 block(TILE_W, TILE_H);
    dim3 grid((width + TILE_W - 1) / TILE_W, (height + TILE_H - 1) / TILE_H);
    size_t sharedBytes = 4 * (TILE_W + 2) * (TILE_H + 2) * sizeof(float);

    _k_computeGradientSum_shared<<<grid, block, sharedBytes>>>(
        d_gx1, d_gy1, d_gx2, d_gy2, ncols, nrows,
        x1, y1, x2, y2, width, height, d_outx, d_outy);
    
    // 5. Wait for kernel to complete
    KLT_CUDA_ASSERT(cudaDeviceSynchronize());

    // 6. Prefetch results back to CPU
    KLT_CUDA_ASSERT(cudaMemPrefetchAsync(d_outx, winBytes, cudaCpuDeviceId, stream));
    KLT_CUDA_ASSERT(cudaMemPrefetchAsync(d_outy, winBytes, cudaCpuDeviceId, stream));

    // 7. Wait for prefetch to CPU to complete
    KLT_CUDA_ASSERT(cudaDeviceSynchronize());

    // 8. Copy from UM (now on host) to output buffers
    memcpy(gradx, d_outx, winBytes);
    memcpy(grady, d_outy, winBytes);

    // 9. Free memory
    cudaFree(d_gx1); cudaFree(d_gy1); cudaFree(d_gx2); cudaFree(d_gy2);
    cudaFree(d_outx); cudaFree(d_outy);
}
...

{
    ...
    float *d_gxx = NULL, *d_gxy = NULL, *d_gyy = NULL;

    size_t bytes = (size_t)N * sizeof(float);
    cudaError_t err;

    /* allocate device arrays for input */
    cudaMalloc((void**)&d_gradx, bytes); 
    cudaMalloc((void**)&d_grady, bytes); 

    /* copy inputs to device */
    cudaMemcpy(d_gradx, gradx, bytes, cudaMemcpyHostToDevice); 
    cudaMemcpy(d_grady, grady, bytes, cudaMemcpyHostToDevice); 

    /* allocate and zero device scalars for results */
    cudaMalloc((void**)&d_gxx, sizeof(float)); 
    cudaMalloc((void**)&d_gxy, sizeof(float)); 
    cudaMalloc((void**)&d_gyy, sizeof(float)); 

    cudaMemset(d_gxx, 0, sizeof(float)); 
    cudaMemset(d_gxy, 0, sizeof(float)); 
    cudaMemset(d_gyy, 0, sizeof(float)); 

    /* launch kernel */
    const int block = 512;
    int grid = (N + block - 1) / block;
    if (grid < 1) grid = 1;
    if (grid > 65535) grid = 65535; /* cap gridDim.x to safe value */

    _compute2by2GradientMatrix_kernel_atomic<<<grid, block>>>(
        d_gradx, d_grady, N, d_gxx, d_gxy, d_gyy);

    err = cudaGetLastError(); 
    err = cudaDeviceSynchronize(); 

    /* copy results back to host */
    err = cudaMemcpy(gxx, d_gxx, sizeof(float), cudaMemcpyDeviceToHost); 
    err = cudaMemcpy(gxy, d_gxy, sizeof(float), cudaMemcpyDeviceToHost); 
    err = cudaMemcpy(gyy, d_gyy, sizeof(float), cudaMemcpyDeviceToHost); 

    /* cleanup */
    cudaFree(d_gradx); cudaFree(d_grady);
    cudaFree(d_gxx); cudaFree(d_gxy); cudaFree(d_gyy);
